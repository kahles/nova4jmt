...
public class Utility {
	 ...
	 * deg.radian
	 */
	static final double D2R = (1.7453292519943295769e-2d);
	/**
	 * radian.deg
	 */
	static final double R2D = (5.7295779513082320877e1d);
	/**
	 * arc seconds per radian
	 */
	static final double R2S = (2.0626480624709635516e5d);
	/**
	 * radians per arc second
	 */
	static final double S2R = (4.8481368110953599359e-6d);
	static final double DMath.PI = (2 * Math.PI);
	static final double RADIAN = (180.0 / Math.PI);
	
	
	

/*! \fn String  ln_get_version (void)
...
double ln_hms_to_deg(LnHms hms)
{
    double degrees;
...
double ln_hms_to_rad(LnHms hms)
{
    double radians;
...
void ln_deg_to_hms (double degrees, LnHms hms)
{
    double dtemp;
...
void ln_rad_to_hms (double radians, LnHms hms)
{
    double degrees;
...
double ln_dms_to_deg(LnDms dms)
{
    double degrees;
...
double ln_dms_to_rad(LnDms dms)
{
    double radians;
...
void ln_deg_to_dms (double degrees, LnDms dms)
{
    double dtemp;
...
void ln_rad_to_dms (double radians, LnDms dms)
{
...
/* add seconds to hms */
void ln_add_secs_hms(LnHms hms, double seconds)
{
    LnHms source_hms;
...
/* add hms to hms */
void ln_add_hms(LnHms source, LnHms dest)
{
...
/*! \fn void ln_hlnlat_to_lnlat(LnhLnlatPosn hpos, LnLnlatPosn pos)
* \brief human readable long/lat position to double long/lat position
* \ingroup conversion
*/
void ln_hlnlat_to_lnlat(LnhLnlatPosn hpos, LnLnlatPosn pos)
{
	pos.lng = ln_dms_to_deg(hpos.lng);
	pos.lat = ln_dms_to_deg(hpos.lat);
}
	
/*! \fn void ln_lnlat_to_hlnlat(LnLnlatPosn pos, LnhLnlatPosn hpos)
* \brief double long/lat position to human readable long/lat position
* \ingroup conversion
*/
void ln_lnlat_to_hlnlat(LnLnlatPosn pos, LnhLnlatPosn hpos)
{
...
/*
* \fn double ln_get_rect_distance(LnRectPosn a, LnRectPosn b)
...
*/
double ln_get_rect_distance(LnRectPosn a, LnRectPosn b)
{
	double x,y,z;
...
int indexOneOf(String string,char...chars) {
...
/*! \fn double ln_get_dec_location(String  s)
...
*
*/
double ln_get_dec_location(String s)
{
	String ptr, dec, tok_ptr;
	...
	if (negative) /* if RA no negative numbers */
		...
		type = Type.LAT;
		if (ame == 0) /* the North/South found before data */
			ptr=ptr.substring(1);
		} else
...
int gettimeofday(TimeVal tv, Timezone tz)
{
	...
	tv.tv_usec = System.currentTimeMillis();
	tv.tv_sec = tv.tv_usec/1000L;
	
	tz.tz_dsttime = TimeZone.getDefault().getOffset(tv.tv_usec);
	tz.tz_minuteswest = TimeZone.getDefault().getRawOffset()/1000L;
...
String strtok_r(String str, String sep, String last)
{
